...
class PhysicalBookingPage extends StatefulWidget {
  const PhysicalBookingPage({super.key});

  @override
  _PhysicalBookingPageState createState() => _PhysicalBookingPageState();
}

class _PhysicalBookingPageState extends State<PhysicalBookingPage> {
  ...
  Future<void> _fetchTimeSlots() async {
    if (_selectedDoctorEmail != null && _selectedDate != null) {
      ...
      final scheduleString = snapshot.data()?['physical_schedule'] ?? '';
      if (scheduleString.isNotEmpty && scheduleString.contains(' - ')) {
        ...
        final bookedSnapshot = await FirebaseFirestore.instance
            .collection('physical_appointments')
    ...
    if (_formKey.currentState!.validate()) {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: const Text('Your physical appointment has been booked successfully.'),
          actions: [
            TextButton(
              onPressed: () {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Book Physical Appointment'),
      ),
      body: _isLoading
